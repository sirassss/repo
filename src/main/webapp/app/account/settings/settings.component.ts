...
export class SettingsComponent implements OnInit {
  @ViewChild('content') public content!: NgbModalRef;
  account!: Account;
  success = false;
  languages = LANGUAGES;
  ...
  payments?: IPayment[];
  listOrder!: MatTableDataSource<IPayment>;
  @ViewChild(MatSort) sortd!: MatSort;
  @ViewChild(MatPaginator) paginator!: MatPaginator;
  isLoadingOrder = false;

  orderLength!: number;
  columns: string[] = ['totalAmount', 'orderAddress', 'orderPhone', 'orderDate', 'status', 'view', 'action'];
  ...
  ngOnInit(): void {
    this.accountService.identity().subscribe(account => {
      if (account) {
        ...
        if (account.imageUrl) {
          this.url = account.imageUrl;
        }
        this.account = account;
        this.getOrder(account);
      }
    });
  }

  getOrder(account: Account) {
    this.paymentService.getPaymentForUser({ login: account.login }).subscribe(res => {
      if (res && res.body) {
        this.payments = res.body;
        this.listOrder = new MatTableDataSource(this.payments);
        this.listOrder.sort = this.sortd;
  ...
  save(): void {
    ...
    this.account.imageUrl = this.url;
  ...
  changeProfile() {
    this.modalRef = this.modalService.open(this.content, { backdrop: 'static', windowClass: 'width-60' });
  }

  cancelPayment(row: IPayment) {
    row.status = 4;
    this.paymentService.update(row).subscribe(
      res => {
        this.toastr.success('Đã hủy, chờ xác nhận');
        this.getOrder(this.account);
      },
      error => {
        this.toastr.error(this.translateService.instant('error.internalServerError'));
      }
